...
def ml_ranked_actions(current, goal_node, model, top_k=2):
...
def hybrid_a_star_planning(start, goal, ox, oy, xy_resolution, yaw_resolution, mode="baseline", model=None, top_k=2):
    """
    start: start node
    goal: goal node
    ox: x position list of Obstacles [m]
...
def run_planner_case(name, start, goal, ox, oy, mode="baseline", model=None, top_k=2):
    t0 = time.perf_counter()
...
def summarize_average(rows):
    if not rows:
        return None
    return {
        "runtime": float(np.mean([r["runtime"] for r in rows])),
        ...
        "switches": float(np.mean([r["switches"] for r in rows])),
    }


def print_average_table(baseline_rows, ml_rows):
    b = summarize_average(baseline_rows)
    m = summarize_average(ml_rows)
    print("\n--- Average Metrics Over Successful Trials ---")
    print(f"{'Method':<18} {'Runtime(s)':>12} {'Nodes':>10} {'Cost':>12} {'Length(m)':>12} {'Switches':>10}")
    print("-" * 80)
    print(f"{'Baseline':<18} {b['runtime']:>12.4f} {b['nodes']:>10.2f} {b['cost']:>12.4f} {b['length']:>12.4f} {b['switches']:>10.2f}")
    print(f"{'ML-guided':<18} {m['runtime']:>12.4f} {m['nodes']:>10.2f} {m['cost']:>12.4f} {m['length']:>12.4f} {m['switches']:>10.2f}")

    print("\n--- Percent Change: ML-guided relative to Baseline ---")
    for key, label in [("runtime", "Runtime"), ("nodes", "Nodes expanded"), ("cost", "Path cost"), ("length", "Path length"), ("switches", "Direction switches")]:
        if abs(b[key]) < 1e-12:
            continue
        pct = 100.0 * (m[key] - b[key]) / b[key]
        print(f"{label:<18}: {pct:>8.2f}%")


def plot_last_success(ox, oy, start, goal, baseline_path, ml_path):
    plt.figure()
    plt.plot(ox, oy, ".k", label="Obstacles")
    if baseline_path and len(baseline_path.x_list) > 0:
        plt.plot(baseline_path.x_list, baseline_path.y_list, "-r", label="Baseline Hybrid A*")
    if ml_path and len(ml_path.x_list) > 0:
        plt.plot(ml_path.x_list, ml_path.y_list, "--b", label="ML-guided Hybrid A*")
    plot_arrow(start[0], start[1], start[2], fc="g")
    plot_arrow(goal[0], goal[1], goal[2], fc="b")
    plt.grid(True)
    plt.axis("equal")
    plt.legend()
    plt.title("Last Successful Trial: Baseline vs ML-guided Hybrid A*")
    plt.savefig("hybrid_astar_multitrial_last_success.png", dpi=200, bbox_inches="tight")
    # plt.show()


def save_results_csv(baseline_rows, ml_rows, filename="hybrid_astar_multitrial_results.csv"):
    import csv
    fieldnames = ["trial", "method", "runtime", "nodes", "cost", "length", "switches"]
    with open(filename, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for row in baseline_rows + ml_rows:
            writer.writerow({k: row[k] for k in fieldnames})
    print(f"Saved CSV: {filename}")


def main():
    print("Start ME 569 multi-trial Hybrid A* comparison")
    print(f"Total actions in baseline: {len(ACTION_SPACE)}")
    print("ML-guided planner uses top_k=3 predicted actions plus straight-motion fallback.\n")

    ox, oy = build_demo_map()

    # Fixed test cases for repeatable results. All are inside the same obstacle map.
    trials = [
        ([10.0, 10.0, np.deg2rad(90.0)],  [50.0, 50.0, np.deg2rad(-90.0)]),
        ([12.0, 12.0, np.deg2rad(90.0)],  [48.0, 48.0, np.deg2rad(-90.0)]),
    ]
    ...
    for i, (start, goal) in enumerate(trials, start=1):
        ...
        try:
            print("Running baseline Hybrid A*...")
            baseline_path, baseline_metrics = run_planner_case(
                "Baseline", start, goal, ox, oy, mode="baseline"
            )
            baseline_metrics["trial"] = i
            if len(baseline_path.x_list) == 0:
                print("Baseline failed for this trial. Skipping ML for this trial.")
                continue

            print("Training Random Forest action selector...")
            ...
            print("Running ML-guided Hybrid A*...")
            ml_path, ml_metrics = run_planner_case(
                "ML-guided", start, goal, ox, oy,
                mode="ml", model=model, top_k=2,
            )
            ml_metrics["trial"] = i
            if len(ml_path.x_list) == 0:
                print("ML-guided planner failed for this trial. Skipping this pair from averages.")
                continue

            print_metrics_table([baseline_metrics, ml_metrics])
            baseline_rows.append(baseline_metrics)
            ml_rows.append(ml_metrics)
            last_success = (start, goal, baseline_path, ml_path)

        except Exception as e:
            print(f"Trial {i} failed with error: {e}")
            continue

    if not baseline_rows or not ml_rows:
        print("\nNo successful paired trials. Try reducing obstacles or changing start/goal pairs.")
        return

    print_average_table(baseline_rows, ml_rows)
    save_results_csv(baseline_rows, ml_rows)

    if last_success is not None:
        start, goal, baseline_path, ml_path = last_success
        plot_last_success(ox, oy, start, goal, baseline_path, ml_path)
        print("Saved figure: hybrid_astar_multitrial_last_success.png")
